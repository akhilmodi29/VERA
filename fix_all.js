...
const prefixMatch = content.split('  return (');
let prefix = prefixMatch[0];

// Remove unused icons
prefix = prefix.replace(/  Clock,\r?\n/g, "");
prefix = prefix.replace(/  Info,\r?\n/g, "");
prefix = prefix.replace(/  Users,\r?\n/g, "");

// Add transcript and voice_integrity_score mapping safely
...
prefix = prefix.replace(/const handleGenerateLiveEvidence = async \(\) => {[\s\S]*?};/g, "/* handleGenerateLiveEvidence */");
prefix = prefix.replace(/const handleEndSession = \(\) => {[\s\S]*?};/g, "/* handleEndSession */");
prefix = prefix.replace(/const activeError = .*?;/g, "/* activeError */");
prefix = prefix.replace(/const voiceIntegrityDisplay = \(\(\) => {[\s\S]*?}\)\(\);/g, "/* voiceIntegrityDisplay */");
prefix = prefix.replace(/const voiceLabelDisplay = \(\(\) => {[\s\S]*?}\)\(\);/g, "/* voiceLabelDisplay */");
prefix = prefix.replace(/const speakerDisplay = \(\(\) => {[\s\S]*?}\)\(\);/g, "/* speakerDisplay */");
prefix = prefix.replace(/const transcriptDisplay = \(\(\) => {[\s\S]*?}\)\(\);/g, "/* transcriptDisplay */");
                ...
                <div className="flex-1 p-5 overflow-y-auto font-mono text-sm leading-relaxed text-gray-300 relative">
                  ...
                  {isProcessing && (
                    <div className="absolute bottom-4 left-5 flex items-center text-[10px] text-vera-accent tracking-widest uppercase bg-[#0A0D14]/90 px-3 py-1.5 rounded-full border border-vera-accent/20">
                      <div className="w-1.5 h-1.5 bg-vera-accent rounded-full animate-ping mr-2" /> {analysisStage !== 'idle' ? STAGE_LABELS[analysisStage] : 'Receiving...'}
                    </div>
                  )}
                </div>
              </div>
...
fs.writeFileSync('frontend/src/pages/Dashboard.tsx', prefix + '  return (\n' + new_jsx);
console.log("Success");
